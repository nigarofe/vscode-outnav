...
function parseOutlineMarkdown(markdown: string, sourceName = 'Outlines.md') {
		...
		for (const raw of lines) {
			...
			if (!line.trim()) { continue; }
				...
				const node: Node = { title, level, children: [] } as any;
				if (actions.length) { (node as any).actions = actions; }
...
function openOutlineWebview(context: vscode.ExtensionContext, jsonUri: vscode.Uri, outlines: any, workspaceRootPath?: string) {
		...
		panel.webview.onDidReceiveMessage(async msg => {
			...
			if (msg.command === 'close') { panel.dispose(); }
				...
				// open external URL
				try { await vscode.env.openExternal(vscode.Uri.parse(msg.url)); }
				catch (e) { console.error('Failed to open url', msg.url, e); }
			} else if (msg.command === 'run_command' && msg.name) {
				try { await vscode.commands.executeCommand(msg.name); }
				catch (e) { console.error('Failed to run command', msg.name, e); }
			}
		});
}

...
export function activate(context: vscode.ExtensionContext) {
  ...
  function outnavFolder() {
    ...
    if (path.basename(workspaceRoot) === 'outnav-workspace') { return workspaceRoot; }
...
function parseOutlinesMarkdown(md: string): OutlineNode {
  ...
  if (!content) { continue; }
        ...
        const node: OutlineNode = { title: content, level, children: [] };
...
function openWebview(context: vscode.ExtensionContext, outlines: any, outnavPath: string) {
    if (panel) {
    ...
    panel.webview.onDidReceiveMessage(async msg => {
        if (msg.command === 'openExternal' && msg.url) {
            try { await vscode.env.openExternal(vscode.Uri.parse(msg.url)); } catch (e) { }
...
function getWebviewHtml(webview: vscode.Webview, extensionUri: vscode.Uri, outlines: any, outnavPath: string) {
    // Use Bootstrap CDN
    ...
    function makeSafeForWebview(srcOutlines: any) {
      if (!srcOutlines || !srcOutlines.document) { return { document: { title: 'root', level: 0, children: [] } }; }

      function cloneNode(n: any): any {
        const copy: any = { title: n.title, level: n.level, children: [] };
        if (n.annotations) {
        ...
        if (n.children && Array.isArray(n.children)) {
          copy.children = n.children.map((c: any) => cloneNode(c));
        }
        return copy;
      }
